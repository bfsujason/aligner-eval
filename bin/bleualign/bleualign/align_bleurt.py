...
def find_first_search_path(src_len, tgt_len):
    search_path = {}
    min_win_size = 10
    max_win_size = 600
    win_per_100 = 8
    ...
    w = min(max(min_win_size, max(win_size_1, win_size_2)), max_win_size)
    for i in range(1, src_len + 1):
        center = int(yx_ratio * i)
        w_start = max(1, center - w)
        w_end = min(center + w, tgt_len)
        search_path[i-1] = [w_start-1, w_end-1]
    return search_path
...
class Aligner:
    ...
    def __init__(self,options):
      ...
      #self.bleurt = bleurt_scorer.BleurtScorer(checkpoint)
      self.bleurt = bleurt_scorer.LengthBatchingBleurtScorer(self.options['bleurt_checkpoint'])
   ...
   #use this if you want to implement your own similarity score
    ...
    def eval_sents(self,translist,targetlist):
      scoredict = {}
      src_len = len(translist)
      tgt_len = len(targetlist)
      search_path = find_first_search_path(src_len, tgt_len)
      for k, v in tqdm(search_path.items()):
        test_id = k
        scores = []
        ref_ids = list(range(v[0], v[1] + 1))
        ref_len = len(ref_ids)
        cand_sents = [translist[k]] * ref_len
        ref_sents = targetlist[v[0]:v[1]+1]
        scores = self.bleurt.score(references=ref_sents, candidates=cand_sents, batch_size=100)
        
        scores = [(score, ref_ids[idx], score) for idx, score in enumerate(scores)]
        scoredict[test_id] = sorted(scores,key=itemgetter(0),reverse=True)[:self.options['maxalternatives']]

      #for testID,testSent in tqdm(enumerate(translist)):
      # print(testID)
      # scores = []
      #  cand_sents = [testSent] * len(targetlist)
      #  scores = self.bleurt.score(references=targetlist, candidates=cand_sents, batch_size=100)
      #  scores = [(score, refID, score) for refID, score in enumerate(scores)]
    ...
    def _eval_sents(self,translist,targetlist):
